...
resource "azurerm_resource_group" "rg_lab20251" {
  name     = "rg-lab20251"
  location = var.location
}

# Servidor MSSQL
resource "azurerm_mssql_server" "sqlsrv_lab20251" {
  name                         = "lab20251sqlsrv"
  resource_group_name          = azurerm_resource_group.rg_lab20251.name
  location                     = azurerm_resource_group.rg_lab20251.location
  version                      = "12.0"
...
resource "azurerm_mssql_database" "db_envio_dw" {
  name         = "envio_dw"
  server_id    = azurerm_mssql_server.sqlsrv_lab20251.id
